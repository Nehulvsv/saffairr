...
export default function ContributorPost() {
    ...
    const fetchPosts = async () => {
      try {
        const res = await fetch(
          `http://localhost:6600/getposts?userId=${userId}`
        );
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:6600/getposts?userId=${userId}&startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:6600/deletepost/${postIdToDelete}/${userId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
