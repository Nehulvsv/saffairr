...
export default function DashUsers() {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:6600/api/user/getusers?startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeleteUser = async () => {
    try {
      const res = await fetch(
        `http://localhost:6600/api/user/delete/${userIdToDelete}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleToggleContributor = async (userId) => {
    try {
      const res = await fetch(
        `http://localhost:6600/api/user/toggleContributor/${userId}`,
        {
          method: "PUT",
          credentials: "include",
        }
      ...
      const data = await res.json();
      if (res.ok) {
        // Update the user list with updated isContributor status
        setUsers((prevUsers) =>
          prevUsers.map((user) =>
            user._id === userId
      ...
      {currentUser.isAdmin && users.length > 0 ? (
        <>
          <Table hoverable className="shadow-md">
            ...
            {users
              .filter((user) => user.isContributor)
              .map((user) => (
